...
// Converte valores no formato brasileiro (1.234,56) ou internacional (1234.56) para número
function parseValorBR(raw) {
  ...
  if (hasComma && hasDot) {
    ...
    // Só vírgula: 1234,56 -> troca por ponto decimal
    str = str.replace(',', '.')
  }
  // Se só tem ponto, já está no formato certo (1234.56)
...
export function parseCSV(content, userRules = {}) {
  ...
  if (!dateCol || !descCol || !valCol) {
    ...
    return rows.map((row, i) => {
      ...
      const rawVal = parseValorBR(vals[2] || vals[1] || 0)
      return normalize(i, vals[0], vals[1], rawVal, null, userRules)
    }).filter(t => t.value !== 0)
  }

    return rows.map((row, i) => {
    const value = parseValorBR(row[valCol])
...
export function parseOFX(content, userRules = {}) {
  ...
  while ((match = txRegex.exec(content)) !== null) {
    ...
    const rawVal = parseValorBR(get('TRNAMT'))
...
export async function parseXLSX(buffer, userRules = {}) {
  ...
  for (let i = startRow; i < rows.length; i++) {
    ...
    const value = parseValorBR(rawVal)
    if (value !== 0) {
